...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
            ...
            $.each(files, function(i, cfile) {
                ...
                for (i = 0; i < browser.clipboard.length; i++)
                    if ((browser.clipboard[i].name == cdata.name) &&
                        ...
                        failed = true
                        msg += cdata.name + ": " + browser.label("File đã được thêm vào Clipboard.") + "\n";
                        break;
                    }
        ...
        if (this.opener.callBack || this.opener.callBackMultiple) {
            html += '<a href="kcact:pick">' + this.label("Chọn") + '</a>';
            if (data.thumb) html +=
                '<a href="kcact:pick_thumb">' + this.label("Chọn Thumbnail") + '</a>';
            html += '<div class="delimiter"></div>';
        }

        if (data.thumb)
            html +='<a href="kcact:view">' + this.label("Xem") + '</a>';

        html +=
            '<a href="kcact:download">' + this.label("Tải về") + '</a>';

        if (!this.readonly) html +=
            '<div class="delimiter"></div>' +
            '<a href="kcact:clpbrdadd">' + this.label("Thêm vào Clipboard") + '</a>' +
            '<div class="delimiter"></div>' +
        ...
        $('.menu a[href="kcact:view"]').click(function() {
            browser.hideDialog();
            $('#loading').html(browser.label("Đang tải ảnh..."));
        ...
        $('.menu a[href="kcact:clpbrdadd"]').click(function() {
            for (i = 0; i < browser.clipboard.length; i++)
                if ((browser.clipboard[i].name == data.name) &&
                    ...
                    browser.hideDialog();
                    alert(browser.label("File đã được thêm vào Clipboard."));
                    return false;
                }
            var cdata = data;
            cdata.dir = browser.dir;
        ...
        $('.menu a[href="kcact:mv"]').click(function(e) {
            ...
            browser.fileNameDialog(
                ...
                'newName', data.name, browser.baseGetData('rename'), {
                    title: "Tên mới:",
                    errEmpty: "Vui lòng điền tên File.",
                    errSlash: "Ký tự không cho phép.",
                    errDot: "Tên file không bắt đầu bằng đấu '.'"
                },
                function() {
                    browser.refresh();
                }
